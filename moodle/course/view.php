...
if ($section && $section > 0) {
    ...
    // Check user is allowed to see it based on completion status.
    if (!$coursesections->uservisible) {
        // Check completion status for the section.
        $sectioncompleted = completion_info::is_section_complete($course, $USER->id, $section);

        // Check if the section is completed or currently in progress.
        if ($sectioncompleted || completion_info::section_requires_completion($course, $USER->id, $section)) {
            $coursesections->uservisible = true;
        } else {
